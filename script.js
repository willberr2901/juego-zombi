...
function playSound(type) {
    try {
        ...
        else if (type === 'MG') { osc.type = 'sawtooth'; osc.frequency.setValueAtTime(600, now); gainNode.gain.setValueAtTime(0.05, now); gainNode.gain.linearRampToValueAtTime(0, now + 0.05); osc.start(now); osc.stop(now + 0.05); }
        ...
        else if (type === 'RELOAD') { osc.type = 'sine'; osc.frequency.setValueAtTime(200, now); osc.frequency.linearRampToValueAtTime(800, now + 0.2); gainNode.gain.setValueAtTime(0.1, now); gainNode.gain.linearRampToValueAtTime(0, now + 0.2); osc.start(now); osc.stop(now + 0.2); }
    } catch (e) {}
}
...
// UI Elements
const menuScreen = document.getElementById('menu-screen'); const gameUI = document.getElementById('game-ui');
const uiScore = document.getElementById('score'); const uiLevel = document.getElementById('level');
const uiAmmo = document.getElementById('ammo'); const uiHealth = document.getElementById('health');
const uiCure = document.getElementById('cure'); const uiLog = document.getElementById('log');
const uiHighScore = document.getElementById('ui-highscore'); const menuHighScore = document.getElementById('menu-highscore');

// RECORD
if (localStorage.getItem('zombieHighScore')) { highScore = parseInt(localStorage.getItem('zombieHighScore')); menuHighScore.innerText = highScore; uiHighScore.innerText = highScore; }
...
class Bullet {
    constructor(tx, ty) { this.x=player.x;this.y=player.y;this.radius=5;this.speed=15;let spread=machineGunActive?(Math.random()-0.5)*0.2:0;let angle=Math.atan2(ty-this.y,tx-this.x)+spread;this.vx=Math.cos(angle)*this.speed;this.vy=Math.sin(angle)*this.speed; }
...
class Zombie {
    constructor() { const s=Math.floor(Math.random()*4); if(s===0){this.x=-64;this.y=Math.random()*canvas.height}else if(s===1){this.x=canvas.width+64;this.y=Math.random()*canvas.height}else if(s===2){this.x=Math.random()*canvas.width;this.y=-64}else{this.x=Math.random()*canvas.width;this.y=canvas.height+64} this.radius=20;this.speed=(Math.random()*1.5+1)+(level*0.2);this.color='#e74c3c'; }
...
const addTouch=(elem,key)=>{elem.addEventListener('touchstart',e=>{e.preventDefault();keys[key]=true;});elem.addEventListener('touchend',e=>{e.preventDefault();keys[key]=false;});elem.addEventListener('mousedown',e=>keys[key]=true);elem.addEventListener('mouseup',e=>keys[key]=false);};
addTouch(btnUp,'w');addTouch(btnDown,'s');addTouch(btnLeft,'a');addTouch(btnRight,'d');
btnFire.addEventListener('touchstart',e=>{e.preventDefault();touchFiring=true;});btnFire.addEventListener('touchend',e=>{e.preventDefault();touchFiring=false;});btnFire.addEventListener('mousedown',()=>touchFiring=true);btnFire.addEventListener('mouseup',()=>touchFiring=false);

function shootBullet(tx,ty) {
    if(machineGunActive){bullets.push(new Bullet(tx,ty));playSound('MG');}
    else{if(ammo>0){bullets.push(new Bullet(tx,ty));ammo--;uiAmmo.innerText=ammo;playSound('SHOOT');}}
}
function handleMobileShooting() {
    if(touchFiring) {
        let tx=player.x,ty=player.y-100; let minDist=9999;
        if(boss){tx=boss.x;ty=boss.y;}
        else if(zombies.length>0){zombies.forEach(z=>{let d=Math.hypot(player.x-z.x,player.y-z.y);if(d<minDist){minDist=d;tx=z.x;ty=z.y;}});}
        if(machineGunActive&&frameCount%5===0)shootBullet(tx,ty);
        else if(!machineGunActive&&frameCount%15===0)shootBullet(tx,ty);
    }
}
...
function startGame() {
    ...
    score=0;level=1;ammo=12;zombies=[];survivors=[];bullets=[];boss=null;particles=[];floatingTexts=[];machineGunActive=false;
...
function endGame(win) {
    ...
    const t=document.getElementById('menu-title');t.innerText=win?"¡VICTORIA!":"GAME OVER";t.style.color=win?"#2ecc71":"#e74c3c";document.getElementById('start-btn').innerText="REINTENTAR";
}
function startSpawners(){clearInterval(spawnZ);clearInterval(spawnS);spawnZ=setInterval(()=>{if(gameRunning&&!boss)zombies.push(new Zombie());},Math.max(500,1500-(level*100)));spawnS=setInterval(()=>{if(gameRunning&&survivors.length<5)survivors.push(new Survivor());},4000);}
...
function update() {
    ...
    for(let i=survivors.length-1;i>=0;i--){
        ...
        if(Math.hypot(player.x-s.x,player.y-s.y)<player.radius+s.radius){
            ...
            survivors.splice(i,1);if(player.cureProgress>=100)endGame(true);
        }
    }
}
...
function animate() {
    ...
    if(roadImg.complete){const p=ctx.createPattern(roadImg,'repeat');ctx.fillStyle=p;ctx.fillRect(0,0,canvas.width,canvas.height);}else ctx.clearRect(0,0,canvas.width,canvas.height);
    update();
    particles.forEach(p=>p.draw());survivors.forEach(s=>s.draw());bullets.forEach(b=>b.draw());
