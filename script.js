/* script.js - V22.0 LÓGICA DEL HUD TÁCTICO */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function playSound(type) {
        ...
        if (type === 'SHOOT') {
            ...
            gain.gain.setValueAtTime(0.2, now); osc.start(now); osc.stop(now + 0.8);
        }
    }
    ...
    // --- FUNCIONES DEL NUEVO HUD ---
    function showAlert(elementId) {
        const alertBox = document.getElementById(elementId);
        alertBox.classList.add('alert-active');
        playSound('ALERT');
        setTimeout(() => { alertBox.classList.remove('alert-active'); }, 3000);
    }

    function levelUp() {
        level++;
        ammo = maxAmmo; // Recompensa: Munición llena
        showAlert('levelup-alert');
        updateHUD();
    }
    ...
    function updateHUD() {
        ...
        // Actualizar Barra de Vida Pro
        const hpPercent = Math.max(0, Math.floor(player.hp));
        document.getElementById("health-bar").style.width = hpPercent + "%";
        document.getElementById("health-num").innerText = hpPercent;
        
        // Efecto de latido si la vida es baja
        const heart = document.querySelector('.heart-icon');
        if(hpPercent < 30) heart.style.animationDuration = "0.4s";
        else heart.style.animationDuration = "0.8s";
    }
    ...
    function update() {
        ...
        for(let i=bullets.length-1; i>=0; i--){
            ...
            if(boss && Math.hypot(b.x-boss.x, b.y-boss.y) < 60) {
                ...
                updateHUD();
                if(boss.hp <= 0) { boss=null; score+=1000; levelUp(); } // Al matar boss, sube nivel
            }
        }
        zombies.forEach(z => {
