/* script.js - HÍBRIDO: PC (TECLADO) + MÓVIL (JOYSTICK) */
...
// --- INPUTS (PC + MÓVIL) ---
const keys = {}; // Para el teclado
let joyX = 0; let joyY = 0; let joyActive = false;
const maxRadius = 50;

// 1. EVENTOS TECLADO (PC)
window.addEventListener('keydown', e => keys[e.key.toLowerCase()] = true);
window.addEventListener('keyup', e => keys[e.key.toLowerCase()] = false);
window.addEventListener('mousedown', e => { if(gameRunning) shootBullet(); }); // Clic para disparar

// 2. EVENTOS JOYSTICK (MÓVIL)
...
function shootBullet() {
    if(ammo > 0) {
        ...
        // Prioridad Joystick
        if (joyX !== 0 || joyY !== 0) { vx = joyX; vy = joyY; }
        // Si no hay Joystick, revisar Teclado (dispara hacia donde te mueves o arriba)
        else {
            if(keys['a']) vx = -1; if(keys['d']) vx = 1;
            if(keys['w']) vy = -1; if(keys['s']) vy = 1;
            if(vx === 0 && vy === 0) vy = -1; // Por defecto arriba
        }

        // Normalizar velocidad bala
        const len = Math.hypot(vx, vy);
        vx = (vx/len) * 15; vy = (vy/len) * 15;
...
function update() {
    // MOVIMIENTO: Sumar Joystick + Teclado
    ...
    // Normalizar teclado para que no corra más rápido en diagonal
    ...
    for(let i=bullets.length-1; i>=0; i--) {
        ...
        for(let j=zombies.length-1; j>=0; j--) {
            ...
            if(Math.hypot(b.x-z.x, b.y-z.y) < 30) {
                ...
                playSound('HIT');
                if(score % 500 === 0) { boss = {x: canvas.width/2, y: -100, hp: 20, maxHp: 20}; uiLog.innerText = "¡JEFE!"; }
                break;
            }
        }
    ...
    if(boss) {
        ...
        for(let i=bullets.length-1; i>=0; i--) {
            ...
            if(Math.hypot(b.x - boss.x, b.y - boss.y) < 60) {
                ...
                if(boss.hp <= 0) { boss = null; score += 1000; uiLog.innerText = "¡JEFE MUERTO!"; }
            }
        }
        if(Math.hypot(player.x - boss.x, player.y - boss.y) < 60) player.health -= 1;
    }

    if(player.health <= 0) { gameRunning = false; alert("INFECTADO. Pts: " + score); location.reload(); }
}
...
function draw() {
    ...
    if(boss && bossImg.complete) {
        ctx.drawImage(bossImg, boss.x-64, boss.y-64, 128, 128);
