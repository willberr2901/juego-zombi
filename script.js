/* script.js - V34.0 PAUSE, LEVELS & REWARDS */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function triggerNuke() {
        ...
        void pulse.offsetWidth; // Reiniciar
        pulse.classList.add("pulse-active");
        playSound('NUKE');

        // EFECTO JUGABLE (EL REGALO)
        zombies = []; // Matar a todos
        if(boss) boss.hp -= 30; // Daño al jefe
        player.hp = Math.min(player.maxHp, player.hp + 50); // Curar
        ammo = maxAmmo; // Recargar balas
    }

    function levelUp() {
        level++;
        killsForNextLevel += 5; // Más difícil el siguiente nivel
        killCount = 0;
        
        triggerNuke(); // ¡DAR EL REGALO!
    ...
    document.getElementById("restart-btn").onclick = () => { document.getElementById("start-btn").click(); togglePause(); };
