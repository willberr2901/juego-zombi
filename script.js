/* script.js - V37.1 SANGRE Y ACCIÓN PRO (FLUJO REORDENADO) */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // MENÚ visible, intro oculta al inicio
    ...
    function update() {
        ...
        else { if (k['d']) dx+=1; if (k['a']) dx-=1; if (k['s']) dy+=1; if (k['w']) dy-=1; }
        ...
        zombies.forEach(z=>{
            ...
            if(!isPaused && Math.hypot(player.x-z.x, player.y-z.y)<30){
        ...
        if(boss){
            ...
            if(!isPaused && Math.hypot(player.x-boss.x, player.y-boss.y)<50){ player.hp-=1; updateHUD(); }
        }
